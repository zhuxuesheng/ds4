...
// VNNI-accelerated Q8_0 batch matmul replacement.
// Quantizes activation to int8, converts Q8_0 weights to uint8 on-the-fly,
// then calls VPDPBUSD batched matmul.
static void ds4_xeon_matmul_q8_0_batch(
        float *out, const ds4_model *m, const ds4_tensor *w,
        const float *x, uint64_t n_tok)
{
    if (!w || w->ndim != 2 || w->type != 8) {  // type 8 = Q8_0
        // Fallback: zero output
        memset(out, 0, (size_t)n_tok * w->dim[1] * sizeof(float));
        return;
    }
    uint64_t in_dim = w->dim[0];
    uint64_t out_dim = w->dim[1];
    const int block_size = 64;
    int n_blocks = (int)((in_dim + block_size - 1) / block_size);

    // Quantize activation to int8 (per-block)
    int8_t *act_i8 = xmalloc((size_t)n_tok * in_dim);
    float *act_scale = xmalloc((size_t)n_tok * n_blocks * sizeof(float));
    ds4_xeon_quantize_a8_per_block(act_i8, act_scale, x, (int)n_tok,
        (int)in_dim, block_size);

    // Convert Q8_0 weights to matmul format (cached per layer is better,
    // but on-the-fly is correct and ~15MB per tensor is tolerable)
    float *ws = NULL;
    uint64_t wo_dim, wi_dim;
    uint8_t *w8 = ds4_xeon_q80_to_matmul(m, w, &ws, &wo_dim, &wi_dim);
    if (!w8) {
        free(act_i8); free(act_scale);
        memset(out, 0, (size_t)n_tok * out_dim * sizeof(float));
        return;
    }

    memset(out, 0, (size_t)n_tok * out_dim * sizeof(float));
    ds4_xeon_matmul_a8w8_vnni_batch(out, act_i8, act_scale,
        w8, ws, (int)n_tok, (int)in_dim, (int)out_dim);
        ...
        const float *steering_dirs, float steering_scale)
{
    ...
    // --- HC pre + RMS norm (CPU, lightweight) ---
    float *attn_residual = xmalloc((size_t)n_tok * hc_dim * sizeof(float));
    float *attn_cur      = xmalloc((size_t)n_tok * DS4_N_EMBD * sizeof(float));
    float *attn_norm     = xmalloc((size_t)n_tok * DS4_N_EMBD * sizeof(float));
    float *post = xmalloc((size_t)n_tok * n_hc * sizeof(float));
    float *comb = xmalloc((size_t)n_tok * n_hc * n_hc * sizeof(float));
    hc_pre_norm_batch(model, layer->hc_attn_fn, layer->hc_attn_scale,
    ...
    for (uint32_t t = 0; t < n_tok; t++) {
        uint32_t pos = (uint32_t)t;  // prefill positions are 0,1,2,...,n_tok-1
        rope_tail_layer_inplace(q + (uint64_t)t * q_dim,
            DS4_N_HEAD, DS4_N_HEAD_DIM, DS4_N_ROT, pos, il, false);
        rope_tail_layer_inplace(kv + (uint64_t)t * DS4_N_HEAD_DIM,
            1, DS4_N_HEAD_DIM, DS4_N_ROT, pos, il, false);
        // FP8 quantize KV + push to cache
        dsv4_fp8_kv_quantize_row_inplace_cpu(kv + (uint64_t)t * DS4_N_HEAD_DIM,
    ...
    layer_grouped_out_batch(attn_out, model, layer, heads, n_tok);
    cpu_directional_steering_project_rows(attn_out, steering_dirs, il,
        n_tok, steering_scale);

    // --- HC post ---
    hc_post_batch(after_attn_hc, attn_out, attn_residual, post, comb,
        n_tok, DS4_N_EMBD, n_hc);
